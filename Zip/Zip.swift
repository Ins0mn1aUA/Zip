...
public class Zip {
    ...
    public class func unzipFileToData(_ zipFilePath: URL, destination: URL, overwrite: Bool, password: String?, progress: ((_ progress: Double) -> ())? = nil, fileOutputHandler: ((_ unzippedFile: URL) -> Void)? = nil) throws -> Data {
        
        // Data array variable
        var arrayForData = [CUnsignedChar]()
        ...
        repeat {
            ...
            while filePointer != nil {
                ...
                if readBytes > 0 {
                    arrayForData.append(contentsOf: buffer)
            ...
            let d = [UInt8](arrayForData)
            
            let pointer = UnsafeBufferPointer(start: d, count: d.count)
            encryptedData = Data(buffer:pointer)
